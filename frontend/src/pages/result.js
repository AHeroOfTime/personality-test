...
export default function Result() {
  ...
  const [getUserAnswers, { data, loading, error }] = useLazyQuery(
    ANSWERS_QUERY,
    {
      onCompleted: (data) => {
  ...
  const gradeQuiz = (groupedAnswers) => {
    ...
    // loop over each item in the obj (grouped by type)
      ...
      <div className="box">
        <div className="content text-center">
          <div className="page-title leading-none -mt-28">
            ...
            {typeDetails?.data?.types[0]?.type &&
              numberSpelledOut[typeDetails?.data?.types[0].type]}
            <br />
            {typeDetails?.data?.types[0]?.subheading &&
              typeDetails?.data?.types[0].subheading}
          </div>
          ...
          <p className="text-left">
            {typeDetails?.data?.types[0]?.description &&
              typeDetails?.data?.types[0].description}
          </p>
        </div>
      </div>
